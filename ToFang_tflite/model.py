...
class Model:
    ...
    def preprocess(self, image):
        ...
        im = tf.image.resize(image, (112,112))
    ...
    def predict(self, image):
        ...
        pred_speed = self.interpreter.get_tensor(self.output_details[1]['index'])[0][0]
        pred_angle = self.interpreter.get_tensor(self.output_details[0]['index'])[0][0]
        speed = np.around(pred_speed).astype(int)*35
        angle = pred_angle*80+50
        if angle > 90:
            angle = angle * 0.98
        else:
            angle = angle * 1.03#1.02
        return angle, speed